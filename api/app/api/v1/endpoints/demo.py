...
async def list_scenarios(db: Session = Depends(deps.get_db)):
    ...
    return db.query(DemoScenario).order_by(DemoScenario.created_at.asc()).all()
...
async def reset_demo(
    ...
    svc = DemoService(db)
    docs = await svc.reset_demo_ready(ai_provider, prompt_registry)
    return {
        "message": "Demo data reset successfully",
        "documents": [
            {"id": doc.id, "title": doc.title, "status": doc.status.value}
            for doc in docs
        ],
    }
